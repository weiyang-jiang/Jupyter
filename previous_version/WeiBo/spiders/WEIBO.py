...
from settings import start_uid, TOPIC
import time
...
class WeiboSpider(scrapy.Spider):
    ...
    # search_url = 'https://weibo.cn/search/mblog?hideSearchFrame=&keyword={特朗普}&page={2'
    search_url = 'https://weibo.cn/search/mblog?hideSearchFrame=&keyword={topic}&advancedfilter=1&endtime={time}&sort=hot'
    # search_url = 'https://weibo.cn/search/mblog?hideSearchFrame=&keyword=%E7%89%B9%E6%9C%97%E6%99%AE&advancedfilter=1&endtime=20200706&sort=hot'
    keyword_user = 6350
    keyword_search = 100
        ...
        if self.pin == 1:
            ...
            for topic in TOPIC:
                for page in range(max_page):
                    ...
                    yield FormRequest(url=self.search_url.format(topic=topic,time=time.strftime("%Y%m%d",time.localtime())),callback=self.parse_search,formdata=form_data)
                    break # 测试代码
    ...
    def parse_search_comment(self,response):
        ...
        item = SearchItem(url_id=url_id,share_count=share_count,comment_count=comment_count,like_count=like_count,author=author,pub_time=pub_time,content_comment=content_comment,url=url)
        return item
    ...
    def parse_comment(self,response):
        ...
        content_comments = response.xpath("//div[@id='M_']//span[@class='ctt']/text()").getall()
        content_comment = "".join(content_comments)
        item = WeiboItem(url_id=url_id,share_count=share_count,comment_count=comment_count,like_count=like_count,author=author,pub_time=pub_time,content_comment=content_comment,url=url)
        return item
